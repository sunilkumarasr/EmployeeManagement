...
class LeaveRequestActivity : AppCompatActivity(), OnItemSelectedListener {
    ...
    fun requestLeave(view: View) {
        ...
        if (is_half_day == 1) {
            Log.e(
                "dateformat2.parse",
                "dateformat2.parse(startDate).date ${
                    dateformat2.parse(startDate).equals(dateformat2.parse(endDate))
                }"
            )
            ...
            if (!dateformat2.parse(startDate).equals(dateformat2.parse(endDate))) {
                Utils.showMessage(
                    "Slected more than 1 day, uncheck Half day checkbox ",
                    applicationContext
                )
                return
            }
        ...
        RetrofitClient.apiInterface.getLeaveTypes(hashMap).enqueue(object : Callback<Any> {
            override fun onResponse(call: Call<Any>, response: Response<Any>) {
                ...
                if (jsobObj.getInt("status") == 200) {
                    ...
                    leaveTypesList = Gson().fromJson<ArrayList<LeaveTypes>>(
                        jsobObj.getJSONArray("leave_types").toString(), leaveLIstType
                    )
            ...
            override fun onFailure(call: Call<Any>, t: Throwable) {
                ...
                val gson = Gson()
                val successResponse =
                    gson.toJson("{\"status\":200,\"message\":\"Data fetched successfully!!\",\"leave_types\":[{\"id\":\"1\",\"leave_type\":\"Casual Leave\"},{\"id\":\"2\",\"leave_type\":\"Sick Leave\"}]}")
                val jsobObj =
                    JSONObject("{\"status\":200,\"message\":\"Data fetched successfully!!\",\"leave_types\":[{\"id\":\"1\",\"leave_type\":\"Casual Leave\"},{\"id\":\"2\",\"leave_type\":\"Sick Leave\"}]}")
                //strRes=strRes.replace("!!","")
                ...
                if (jsobObj.getInt("status") == 200) {
                    val leaveLIstType: Type = object : TypeToken<ArrayList<LeaveTypes>>() {}.type
                    leaveTypesList = Gson().fromJson<ArrayList<LeaveTypes>>(
                        "[{\"id\":\"1\",\"leave_type\":\"Casual Leave\"},{\"id\":\"2\",\"leave_type\":\"Sick Leave\"}]",
                        leaveLIstType
                    )
                    leaveTypeAdapter.addLeaveTypes(leaveTypesList)
                }
